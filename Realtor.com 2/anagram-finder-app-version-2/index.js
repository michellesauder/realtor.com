...
var readFile = getAnagrams.prototype.readFile = function () {
    ...
    fs.readFile('dictionary.txt', 'utf8', (err, data) => {
        ...
        rl.on('line', (word) => {
            if(word !== 'exit'){
                console.time('timer');
                var isEmpty = word.split("").length == 0;
                if(findAnagrams(word) && !!!isEmpty) {
                    var wordsFound = findAnagrams(word).join(", ");
                    console.log(findAnagrams(word).length + ` Anagrams found for ${word} in `);
                    console.timeEnd('timer');
                    console.log(wordsFound);
                }else if(isEmpty){
                    console.log('\x1b[33m%s\x1b[0m', `Please input a valid word`); //correction found
                    console.timeEnd('timer');
                }else{
                    console.log('\x1b[33m%s\x1b[0m', `No Anagrams found for ${word}`); //correction found
                    console.timeEnd('timer');
                }
                console.log("\x1b[32m")
                rl.prompt();
